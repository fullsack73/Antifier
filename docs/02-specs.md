...
- `POST /api/optimize-portfolio`: 포트폴리오 최적화 job 시작. 같은 `request_id` 재요청은 기존 job 상태를 반환합니다. 완료/데이터 오류 결과의 `data_eligibility`는 `minimum_observations`, 14일 staleness, no-leading-fill alignment 정책과 ticker별 `no_price_data`, `insufficient_history`, `stale_price`, `fx_unavailable`, `invalid_price`, `forecast_output_missing`, `alignment_missing` 사유를 제공합니다.
  - 비율 입력은 모두 decimal입니다. 기존 요청 호환을 위해 `max_asset_weight` 생략 시 20%를 목표로 하되 적격 종목이 5개 미만이면 기존과 같이 `1/N`까지의 feasible cap을 사용합니다. 사용자가 `max_asset_weight`를 명시하면 자동 완화하지 않습니다. `l2_gamma=0`, `min_holding_weight=0`, `turnover_penalty=0`이며 `rebalance_band`, `max_turnover`, `current_weights`는 선택 입력입니다. `turnover_penalty`는 `current_weights`가 있을 때만 L1 objective로 적용합니다.
  - 종목별 hard constraint는 `asset_constraints: [{"ticker":"AAPL","min_weight":0.05,"max_weight":0.20}]`, 그룹 hard constraint는 `group_constraints: [{"dimension":"sector|industry|country","group":"Technology","min_weight":0.10,"max_weight":0.40}]` 형식을 사용합니다. 생략한 bound는 적용하지 않으며 기존 요청 shape는 그대로 유효합니다. 전체 공통 cap은 `max_asset_weight`를 사용합니다.
  - hard constraint는 MIN_VARIANCE, BL, MPT와 target-return/risk 경로의 solver에 같은 의미로 직접 적용합니다. 종목별 bound와 그룹 selector는 유효 universe에 맞춰 정규화하고, lower/upper 합계, 공통 cap, 그룹 충돌, 종목-그룹 충돌과 목표 수익률/위험의 명백한 infeasibility를 본 최적화 전에 검사합니다. 기존처럼 infeasible `max_asset_weight`를 자동 완화하지 않습니다.
  - 그룹 분류 source는 near-live 실행의 `yfinance.info`입니다. 응답 `classification_metadata`는 source, as-of, requested dimensions, ticker별 분류, coverage, 누락 ticker와 status를 포함합니다. 관련 분류가 하나라도 없거나 upstream 호출이 실패하면 그룹 제약을 조용히 축소하지 않고 `METADATA_UNAVAILABLE`로 실패합니다. 최신 분류를 과거 시점 분류처럼 사용하지 않으므로 종료일이 현재 기준 14일 밖인 historical optimization의 그룹 제약은 point-in-time source가 추가되기 전까지 `POINT_IN_TIME_METADATA_REQUIRED`로 실패합니다.
  - `min_holding_weight` 후처리와 rebalance/turnover control 뒤에도 hard constraint를 재검사합니다. 반환 weight가 제약을 만족하지 못하면 위반 weight를 성공 결과로 내리지 않고 구조화된 infeasibility를 반환합니다. raw optimizer와 실제 반환 weight는 `pre_control_weights`, `controlled_weights`, 현금/미모델 노출 필드로 구분합니다.
  - 성공 응답의 `risk_diagnostics`는 최종 반환 weight 기준 marginal/percentage risk contribution, HHI, effective holdings, maximum weight, covariance condition number/effective rank/average pairwise correlation, 계산 coverage/status/reason을 포함합니다. near-live 분류가 있으면 sector/industry/country exposure도 포함합니다. `constraint_diagnostics`는 constraint별 actual, lower/upper, slack, binding, satisfied와 전체 status를 포함합니다.
  - validation/solver 오류는 내부 traceback이나 solver 문자열을 노출하지 않고 `error`, `error_code`, `constraint`, `requested_value`, `feasible_bound`, `affected_tickers`, `affected_groups`를 가능한 범위에서 제공합니다. background job/SSE 실패도 같은 구조화 필드를 유지합니다.
  - band/turnover control 후 위험자산 weight 합계가 1 미만이면 차이를 현금으로 보존합니다. 응답의 `risky_exposure`, `cash_weight`, `controlled_cash_weight`를 사용하며, 표시 expected return은 현금에 risk-free return을 적용하고 volatility는 실제 위험자산 노출로 계산합니다. 성과 계산을 위해 위험자산을 암묵적으로 100% 재정규화하지 않습니다.
  - turnover control 때문에 expected-return/covariance universe에서 제외된 기존 보유가 남으면 이를 현금으로 오인하지 않습니다. `unmodeled_weights`, `unmodeled_risky_exposure`, `performance_coverage`를 노출하고 `performance_status=unavailable_unmodeled_exposure`와 함께 return/risk/Sharpe를 `null`로 반환합니다. UI는 숫자를 만들지 않고 계산 불가 경고를 표시합니다.
  - 투자금 배분 UI는 positive target weight를 가진 모든 ticker에 유효한 양수 가격이 있을 때만 share plan을 계산합니다. 가격 누락 ticker에 `$1` fallback을 사용하지 않으며, 누락 ticker 목록을 표시하고 allocation을 차단합니다. 투자 예산도 양수 finite 값이어야 합니다.
...
- `tools/backtest_portfolio_models.py`: CSV, ticker list, ticker group을 입력받아 rolling rebalance backtest를 실행하고 `settings`, `models`, `summary_by_model`, `alpha_diagnostics`, `rebalance_records`, `promotion_decision`, `warnings` JSON을 저장합니다. 모델에는 `risk_parity`, `momentum_6m`, `low_volatility`, `market_cap_weight`, `momentum_12_1`, `momentum_bl`, `signal_stack_bl`, `adaptive_signal_tilt`, `calibrated_lightweight_bl`, `arima_transformer_rank_bl`, `transformer_rank_bl`이 포함됩니다.
  ...
  - `tools/validate_pooled_patch_transformer.py`는 fresh validation 전용입니다. OHLCV, dated universe, PIT factor, Kronos revision/inference batch, Patch Transformer config/seed와 실행 제약을 immutable split manifest로 검증하고, active universe 전체를 coverage 분모로 사용합니다. Frozen candidate가 absolute signal gate와 pooled ridge/frozen Kronos 대비 paired rank-IC·top-bottom spread 95% gate를 모두 통과한 경우에만 5% active-share Ledoit-Wolf GMV overlay를 10 bps 비용, 2% rebalance band, 35% turnover cap으로 검증합니다.
  ...
  - research-only `factor_model_minimum_variance`는 공식 French 일별 `Mkt-RF`, `SMB`, `HML`, `RF`와 signal date 이전 504거래일 종목 수익률만 사용해 `B F B' + D` covariance를 만듭니다. `B`는 excess-return FF3 OLS, `F`는 half-life 63일 EWMA에 25% diagonal shrinkage, `D`는 회귀 잔차분산에 50% cross-sectional median shrinkage와 daily `1e-8` floor를 적용하며 전체 covariance는 대칭화 후 spectral PSD floor `1e-12`를 적용합니다.
  - FF3 factor data 부족, 252개 미만 정렬 관측, rank deficiency, 종목 회귀/비정상 specific variance/covariance 실패 또는 solver 실패는 같은 origin의 Ledoit-Wolf GMV weight로 정확히 fallback합니다. diagnostics는 사유, factor exposure, specific variance, covariance condition/effective rank와 fallback 여부를 기록합니다.
  - factor-risk baseline/candidate 비교도 universe, origin, 63일 rebalance, long-only 20% cap, 10 bps 비용, 2% band, 35% turnover cap과 historical risk-free series를 공유합니다. consumed origin manifest는 `promotion_use=forbidden_non_promotion_smoke`로 잠가 통계 결과와 무관하게 승격할 수 없습니다.
  - 고정 FF3 factor-risk 사양은 동결 후 처음 개봉한 2012-2018 French 12-industry 미사용 historical holdout의 완전한 63일 origin 27개에서 Ledoit-Wolf GMV와 비교했습니다. 변동성은 10.5515%에서 10.5426%로 0.89 bp 낮아졌지만 Sharpe는 1.1748에서 1.1718로 낮아졌고 paired 확률 66.5%/43.2%와 Holm gate를 통과하지 못했으므로 production 기본값으로 승격하지 않습니다. 이 검증은 live post-2025 forward collection이 아니라 사전 미사용 과거 구간을 사양 동결 후 1회 실행한 결과입니다.
