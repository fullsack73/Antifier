#!/usr/bin/env python3
"""Evaluate fixed risk-parity and momentum construction diversification."""
...
def _settings(args):
    return {
        ...
        "construction_policy": {
            ...
            "risk_component": "inverse_volatility_risk_parity",
...
def _joint_gate(
    ...
    if signal_result["gate"]["status"] != "passed":
        reasons.extend(
            f"Momentum signal: {reason}"
            for reason in signal_result["gate"]["reasons"]
        )
    signal_probability = signal_result["bootstrap"].get(
        "probability",
        {},
    )
    hypotheses = {
        "signal:positive_mean_rank_ic": signal_probability.get(
            "positive_mean_rank_ic"
        ),
        "signal:positive_mean_top_bottom_spread": (
