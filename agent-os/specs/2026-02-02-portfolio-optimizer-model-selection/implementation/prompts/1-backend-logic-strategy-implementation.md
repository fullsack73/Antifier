We're continuing our implementation of portfolio-optimizer-model-selection by implementing task group number 1:
...
- [ ] 1.0 Complete Backend Strategy Logic
  - [ ] 1.1 Write 2-8 focused tests for `optimize_portfolio` strategy selection
  ...
  - [ ] 1.2 Refactor `optimize_portfolio` signature in `src/backend/portfolio_optimization.py`
    - Accept `model_strategy` parameter (default: "BL")
    - Update caching decorator key generation if needed to include strategy
  - [ ] 1.3 Implement "Classic MPT" strategy logic
  ...
  - [ ] 1.4 Implement "Deep Learning Ensemble" strategy logic
...
Read @agent-os/specs/2026-02-02-portfolio-optimizer-model-selection/spec.md to understand the context for this spec and where the current task fits into it.
...
For example, if you've been assigned implement the 3rd task from `tasks.md` and that task's title is "Commenting System", then you must create the file: `agent-os/specs/2026-02-02-portfolio-optimizer-model-selection/implementation/3-commenting-system-implementation.md`.
...
## Overview
**Task Reference:** Task #[number] from `agent-os/specs/2026-02-02-portfolio-optimizer-model-selection/tasks.md`
