We're continuing our implementation of performance-optimization-black-litterman by implementing task group number 2:
...
- [ ] 2.0 Tune Parallelism Parameters
  - [ ] 2.1 Write a parameterized benchmark test
    - Script that runs `ml_forecast_returns` with different `batch_size` and `max_workers`
  - [ ] 2.2 Optimize `max_workers` calculation in `portfolio_optimization.py`
