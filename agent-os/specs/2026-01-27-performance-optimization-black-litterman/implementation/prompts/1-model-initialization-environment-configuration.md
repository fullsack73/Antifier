We're continuing our implementation of performance-optimization-black-litterman by implementing task group number 1:
...
- [ ] 1.0 Optimize model initialization and threading
  - [ ] 1.1 Write 2 performance benchmark tests/scripts
  ...
  - [ ] 1.2 Implement `worker_initializer` in `portfolio_optimization.py`
  ...
  - [ ] 1.3 Modify `LSTMModel` in `forecast_models.py`
  ...
  - [ ] 1.4 Modify `XGBoostModel` in `forecast_models.py`
    - Update `XGBRegressor` initialization to use `n_jobs=1` or `nthread=1`
  - [ ] 1.5 Validate configuration application
...
Read @agent-os/specs/2026-01-27-performance-optimization-black-litterman/spec.md to understand the context for this spec and where the current task fits into it.
...
For example, if you've been assigned implement the 3rd task from `tasks.md` and that task's title is "Commenting System", then you must create the file: `agent-os/specs/2026-01-27-performance-optimization-black-litterman/implementation/3-commenting-system-implementation.md`.
...
## Overview
**Task Reference:** Task #[number] from `agent-os/specs/2026-01-27-performance-optimization-black-litterman/tasks.md`
