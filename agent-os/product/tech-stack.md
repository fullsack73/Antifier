...
- **Regression Models**: LightGBM, XGBoost (gradient boosting), TensorFlow, Keras
...
## HTTP & API
- **HTTP Client**: Native fetch with AbortController (primary), Axios 1.8.4 (secondary)
...
- **Data Storage**: In-memory caching for stock data; JSON file persistence for portfolio results
...
## Architecture Notes
- **Design Pattern**: Single Page Application (SPA) with React frontend (State-based routing, no external router)
